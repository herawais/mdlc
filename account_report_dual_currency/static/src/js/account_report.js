...
console.log("loadeddddddddddddddddddddddd")
...
var M2MFilters = Widget.extend(StandaloneFieldManagerMixin, {
    ...
    willStart: function () {
        ...
        _.each(this.fields, function (field, fieldName) {
            ...
            console.log(self,field,"fielffffffffffffffffffffffffff",fieldName)
        });
...
var M2OFilters = Widget.extend(StandaloneFieldManagerMixin, {
    ...
    willStart: function () {
        ...
        _.each(this.fields, function (field, fieldName) {
            ...
            console.log(self,field,"M2OFiltersM2OFiltersM2OFiltersM2OFilters",fieldName)
        });
        return Promise.all(defs);
    },
    /**
    ...
    _makeM2OWidget: function (fieldInfo, fieldName) {
        var self = this;
        let domain1 = [['active','=',true]]
        let getActiveCurrency = this._rpc({
            model: 'res.currency',
            method: 'search',
            args: [domain1,[]],
            // kwargs: {limit: 1}
        });
        console.log(this,"getActiveCurrency----------------",getActiveCurrency)
        const fieldSetup = getActiveCurrency.then(TaxesIds => {
            return this.model.makeRecord('res.currency', [{
                name: 'active_currency',
                type: 'many2one',
                relation: 'res.currency',
                domain : domain1,
                value: self.active_currency,
                fields: [{
                    name: 'id',
                    type: 'integer',
                }, {
            ...
            self.active_currency = new FieldMany2One(self, 'active_currency', record, {
                mode: 'edit',
                attrs: {
                    can_create: false,
                    can_write: false,
            ...
            this._registerWidget(recordID, 'active_currency',this.active_currency);
        });
...
var accountReportsWidget = AbstractAction.extend({
    ...
    custom_events: {
        ...
        'currency_field_changed': function(ev) {
             var self = this;
             self.report_options.other_currency = ev.data.other_currency;
             return self.reload().then(function () {
                 self.$searchview_buttons.find('.account_currency_field').click();
             });
        },
    ...
    _onFieldChanged: function (event) {
        return StandaloneFieldManagerMixin._onFieldChanged.apply(this, arguments).then(()=>{
            ...
            //     this.other_currency = event.data.changes.active_currency.id;
            // }
            // this._reload();
        });
    ...
    render_searchview_buttons: function() {
        ...
        console.log(this.$searchview_buttons,"dddddddddddddddddddddddddddddddd",this.$searchview_buttons.find('.js_currency_m2o'))
        this.active_currency = new M2OFilters(this, fields, 'currency_field_changed');
        this.active_currency.appendTo(this.$searchview_buttons.find('.js_currency_m2o'));
