...
var GeneratePriceList = AbstractAction.extend(StandaloneFieldManagerMixin, {
    ...
    init: function (parent, params) {
        ...
        this.context.taxes_ids = [];
        this.context.parent_categ_ids = [];
    },
    ...
    willStart: function () {
        ...
        let domain = [['type_tax_use','=','sale']]
        let getCustomerTaxes = this._rpc({
            model: 'account.tax',
            method: 'search',
            args: [domain,[]],
            kwargs: {limit: 1}
        ...
        const fieldSetup1 = getCustomerTaxes.then(TaxesIds => {
            return this.model.makeRecord('report.product.report_pricelist', [{
                name: 'taxes_id',
                type: 'many2many',
            ...
            self.taxesHandle = recordID;
            const record = this.model.get(recordID);
            self.tax_ids = new FieldMany2ManyTags(this, 'taxes_id', record, {
                mode: 'edit',
                attrs: {
                    can_create: false,
                    can_write: false,
            ...
            this._registerWidget(recordID, 'taxes_id', this.tax_ids);
        });

        let domain1 = [['parent_id','=',false]]
        let getParentCategs = this._rpc({
        ...
        const fieldSetup2 = getParentCategs.then(TaxesIds => {
            return this.model.makeRecord('report.product.report_pricelist', [{
                name: 'parent_categs',
                type: 'many2many',
                relation: 'product.category',
                domain : domain1,
                value: self.parent_categs,
                fields: [{
                    name: 'id',
                    type: 'integer',
                }, {
            ...
            self.parentCategHandle = recordID;
            const record = this.model.get(recordID);
            self.parent_categs = new FieldMany2ManyTags(this, 'parent_categs', record, {
                mode: 'edit',
                attrs: {
                    can_create: false,
                    can_write: false,
            ...
            this._registerWidget(recordID, 'parent_categs',this.parent_categs);
        });


        return Promise.all([fieldSetup,fieldSetup1,fieldSetup2, this._getHtml(), this._super()]);
    },
    /**
     * @override
     */
    ...
    _renderComponent: function () {
        ...
        this.tax_ids.appendTo($searchview.find('.o_taxes'));
        this.parent_categs.appendTo($searchview.find('.o_categs'));
    ...
    _onClickPrint: function () {
        ...
        const reportName = _.str.sprintf('product.report_pricelist?active_model=%s&active_ids=%s&pricelist_id=%s&quantities=%s&taxes_ids=%s&parent_categ_ids=%s',
            this.context.active_model,
            this.context.active_ids || '',
            this.context.pricelist_id || '',
            this.context.quantities.toString() || '1',
            this.context.taxes_ids.toString() || [],
            this.context.parent_categ_ids.toString() || [],
        );
    ...
    _onFieldChanged: function (event) {
        return StandaloneFieldManagerMixin._onFieldChanged.apply(this, arguments).then(()=>{
            if(event.target.formatType == "many2one"){
                this.context.pricelist_id = event.data.changes.pricelist_id.id;
            }
            if(event.target.formatType == "many2many"){
                if(event.target.name == "parent_categs"){
                    var record = this.model.get(this.parentCategHandle);
                    this.context.parent_categ_ids = record.data.parent_categs.res_ids;
                }else{
                    var record = this.model.get(this.taxesHandle);
                    this.context.taxes_ids = record.data.taxes_id.res_ids;
                }
