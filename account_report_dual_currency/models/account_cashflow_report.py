...
class CashFlowReportCustomHandler(models.AbstractModel):
    ...
    def _add_report_data(self, layout_line_id, aml_data, layout_data, report_data):
        ...
        """
        def _report_update_parent(layout_line_id, aml_column_group_key, aml_balance, aml_balance_currency, layout_data, report_data):
            ...
            if 'parent_line_id' in layout_data[layout_line_id]:
                ...
                report_data.setdefault(parent_line_id, {'balance': {}, 'balance_currency': {}})
                ...
                report_data[parent_line_id]['balance_currency'].setdefault(aml_column_group_key, 0.0)
                report_data[parent_line_id]['balance_currency'][aml_column_group_key] += aml_balance_currency

                
                _report_update_parent(parent_line_id, aml_column_group_key, aml_balance, aml_balance_currency, layout_data, report_data)
        ...
        aml_balance_currency = aml_data['balance_currency']
        ...
        report_data[layout_line_id]['balance_currency'].setdefault(aml_column_group_key, 0.0)
        report_data[layout_line_id]['balance_currency'][aml_column_group_key] += aml_balance_currency
        ...
        report_data[layout_line_id]['aml_groupby_account'][aml_account_id]['balance_currency'].setdefault(aml_column_group_key, 0.0)
        report_data[layout_line_id]['aml_groupby_account'][aml_account_id]['balance_currency'][aml_column_group_key] += aml_balance_currency

        _report_update_parent(layout_line_id, aml_column_group_key, aml_balance, aml_balance_currency, layout_data, report_data)
    ...
    def _dispatch_aml_data(self, tags_ids, aml_data, layout_data, report_data):
        ...
        # elif aml_data['balance_currency'] < 0:
        #     if aml_data['account_tag_id'] == tags_ids['operating']:
        #         self._add_report_data('paid_operating_activities', aml_data, layout_data, report_data)
        #     elif aml_data['account_tag_id'] == tags_ids['investing']:
        #         self._add_report_data('investing_activities_cash_out', aml_data, layout_data, report_data)
    ...
    def _compute_liquidity_balance(self, report, options, currency_table_query, payment_account_ids, date_scope):
        ...
        other_currency_table_query = self.env['res.currency']._get_query_other_currency_table(options)
        # print('--------==>>> other_currency_table_query', other_currency_table_query)
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            queries.append(f'''
                SELECT
                    ...
                    SUM(ROUND(account_move_line.balance * other_currency_table.rate, other_currency_table.precision)) AS balance_currency
                FROM {tables}
                ...
                LEFT JOIN {other_currency_table_query}
                    ON other_currency_table.company_id = account_move_line.company_id
                WHERE {where_clause}
    ...
    def _get_liquidity_moves(self, report, options, currency_table_query, payment_account_ids, payment_move_ids):
        ...
        # print('--------==>>> currency_table_query', currency_table_query)
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            queries.append(f'''
                ...
                SELECT
                    ...
                    SUM(ROUND(account_partial_reconcile.amount * other_currency_table.rate, other_currency_table.precision)) AS balance_currency
        ...
        for aml_data in self._cr.dictfetchall():
            ...
            reconciled_aml_groupby_account[aml_data['account_id']][aml_data['column_group_key']]['balance_currency'] -= aml_data['balance_currency']
    ...
    def _get_reconciled_moves(self, report, options, currency_table_query, payment_account_ids, payment_move_ids):
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            queries.append('''
                SELECT
                    ...
                    SUM(account_partial_reconcile.amount) AS balance_currency
        ...
        for aml_data in self._cr.dictfetchall():
            ...
            reconciled_percentage_per_move[aml_data['column_group_key']][aml_data['move_id']][aml_data['account_id']][0] += aml_data['balance_currency']
        ...
        for column in options['columns']:
            queries.append(f'''
                SELECT
                    ...
                    SUM(account_move_line.balance) AS balance_currency
                FROM account_move_line
        ...
        for aml_data in self._cr.dictfetchall():
            if aml_data['account_id'] in reconciled_percentage_per_move[aml_data['column_group_key']][aml_data['move_id']]:
                ...
                reconciled_percentage_per_move[aml_data['column_group_key']][aml_data['move_id']][aml_data['account_id']][1] += aml_data['balance_currency']
        ...
        for aml_data in self._cr.dictfetchall():
            ...
            total_amount = 0.0
            # print('--------==>>> reconciled_percentage_per_move', reconciled_percentage_per_move)
            for reconciled_amount, amount in reconciled_percentage_per_move[aml_column_group_key][aml_move_id].values():
                # print('--------==>>> reconciled_amount, amount', reconciled_amount, amount)
            ...
            reconciled_aml_per_account[aml_account_id][aml_column_group_key]['balance_currency'] -= aml_balance
    ...
    def _get_layout_line(self, report, options, layout_line_id, layout_line_data, report_data):
        ...
        for column in options['columns']:
            ...
            if column.get('name') == 'Currency Balance':
                currency = self.env['res.currency'].browse((options.get('currency')))

            column_values.append({
                'name': report.format_value(value, currency=currency, blank_if_zero=column['blank_if_zero'], figure_type=column['figure_type']),
                'no_format': value,
                'class': 'number',
            })
    ...
    def _get_unexplained_difference_line(self, report, options, report_data):
        ...
        for column in options['columns']:
            ...
            column_values.append({
                'name': report.format_value(delta, currency=currency, blank_if_zero=False, figure_type='monetary'),
                'no_format': delta,
                'class': 'number',
            })
